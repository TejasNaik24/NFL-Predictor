...
# -------------------------
# INFERENCE-ONLY FUNCTIONS (No CSV dependency)
# -------------------------

def run_automl_bracket_inference(
    model1,
    model2,
    precomputed_team_stats: pd.DataFrame,
    precomputed_elo_ratings: Dict[str, float],
    current_season: int,
    progress_callback: Optional[callable] = None
) -> Dict[str, Any]:
    """
    Run the full AutoML bracket prediction using PRECOMPUTED features.
    
    This is the inference-only version for deployed models that do not require CSV files.
    
    Args:
        model1: Trained playoff qualifier model
        model2: Trained bracket predictor model
        precomputed_team_stats: Pre-calculated team statistics DataFrame
        precomputed_elo_ratings: Pre-calculated ELO ratings dict
        current_season: Season year (for metadata only)
        progress_callback: Optional callback for progress updates
        
    Returns:
        Dict with bracket results (same structure as run_automl_bracket)
    """
    if model1 is None:
        raise ValueError("Model 1 (Playoff Qualifier) is required")
    if model2 is None:
        raise ValueError("Model 2 (Bracket Predictor) is required")
    if precomputed_team_stats is None:
        raise ValueError("Precomputed team stats are required for inference")
    if precomputed_elo_ratings is None:
        raise ValueError("Precomputed ELO ratings are required for inference")
    ...
    # Step 1: Use precomputed features (no computation needed)
    if progress_callback:
        progress_callback("Using precomputed team statistics...", "info")
    
    team_stats = precomputed_team_stats
    elo_dict = precomputed_elo_ratings
...
def run_manual_bracket_inference(
    model2,
    precomputed_team_stats: pd.DataFrame,
    precomputed_elo_ratings: Dict[str, float],
    current_season: int,
    ...
    """
    Run bracket prediction using user-selected teams and PRECOMPUTED features.
    ...
    Returns:
        Dict with bracket results (same structure as run_manual_bracket)
    """
    if model2 is None:
        raise ValueError("Model 2 (Bracket Predictor) is required")
    if precomputed_team_stats is None:
    ...
    # Use precomputed features (no computation needed)
    if progress_callback:
        progress_callback("Using precomputed team statistics...", "info")
