...
# Load data
# -------------------------
print("Loading CSVs from data_files/ ...")
df_full = merge_data("data_files")
df_full = ensure_points(df_full)
print(f"Loaded {len(df_full)} rows. Seasons found: {sorted(df_full['season'].dropna().unique())}")

# -------------------------
# Model 1: Playoff Qualifier (season aggregates)
# -------------------------
print("\n=== TRAIN MODEL 1: Playoff Qualifier ===")
...
# -------------------------
print("\n=== TRAIN MODEL 2: Playoff Game Winner ===")
playoff_df = df_full[df_full['season_type'].str.lower() == 'post'].copy()
if playoff_df.empty:
    print("Warning: no playoff rows found (season_type == 'post'). Model 2 will be skipped.")
    model2 = None
    final_feature_cols = []
else:
    ...
    if df_reg_full.empty:
        print("Warning: no regular-season rows available to compute recent-form. Filling zeros.")
...
# -------------------------
# Save models into models/model_START-END
# -------------------------
print("\n=== SAVING MODELS ===")
base_dir = "models"
if not os.path.exists(base_dir):
    print("models folder not found, creating one...")
    os.makedirs(base_dir, exist_ok=True)
else:
    print("found models folder, saving model version inside...")
...
if not os.path.exists(version_path):
    print(f"version folder '{version_folder}' not found, creating it...")
    os.makedirs(version_path, exist_ok=True)
else:
    print(f"version folder '{version_folder}' already exists — overwriting model files inside.")

# save Model 1
m1_file = os.path.join(version_path, "model1_playoff_qualifier.pkl")
joblib.dump(model1, m1_file)
print(f"Saved Model 1 -> {m1_file}")

# save Model 2 if it exists
if 'model2' in locals() and model2 is not None:
    m2_file = os.path.join(version_path, "model2_bracket.pkl")
    joblib.dump(model2, m2_file)
    print(f"Saved Model 2 -> {m2_file}")
else:
    print("Model 2 was not trained (no playoff rows). Skipping saving Model 2.")

print("\nDone. Models saved in:", version_path)
